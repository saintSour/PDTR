...
void BattleStart(maincharacter& mainCharacter, std::vector<Enemy>& enemies) //сделать мгновенную смерть
{
	...
	{
		mainCharacter.characteristics.initiative = 7; //временна€ проверка
		...
		for (int i = enemies.size(); i > 0; i--)
		{
			...
			else 
			{
				if (i == 1)
				{
					...
					else if (!enemies.empty()) { enemies[(i - 1)].standartPhysAttack(enemies[(i - 1)], mainCharacter); EnemyDeath(mainCharacter, enemies); }//логика реб€т
				}
			}
		}
		EnemyDeath(mainCharacter, enemies);
